...
| type | 필드 | 설명 |
|---|---|---|
| `room-created` | `roomCode` | 방 생성 완료, 발급된 코드. 이 연결 자체는 참여자로 등록되지 않고 곧 끊긴다(아래 "방 생성/삭제 흐름 참고") |
...
### 방 생성/삭제 흐름 참고
`create-room`을 보낸 연결(런처)은 참여자로 등록되지 않는다 — 코드만 받아서 보여준 뒤 바로
끊기기 때문이다. 그래서 방은 "참여자 0명"인 상태로 생성되며, 이후 실제 캐릭터 창이
`join-room`으로 접속해야 비로소 진짜 참여자가 생긴다. 방을 만든 사람이 코드를 친구에게
알려주고 친구가 그 코드를 입력하기까지는 실제로 수십 초~수 분이 걸릴 수 있으므로,
`EMPTY_ROOM_GRACE_MS` 동안은 참여자가 0명이어도 방을 지우지 않고 기다린다. 이 시간이
지나도록 아무도 join하지 않은 방만 실제로 삭제된다. (예전엔 이 유예시간을 "런처→캐릭터 창
전환 사이의 아주 짧은 순간"만 버티면 된다고 보고 15초로 잡았었는데, 실사용 중 코드 공유에
걸리는 실제 시간이 그보다 길어서 멀쩡한 방이 먼저 삭제되는 버그가 발견되어 10분으로 늘림.)
